...
#include <stdio.h>
/*Вариант 17
Написать программу для работы по запросам оператора с упорядоченной таблицей, реализованной в виде 2,3,4-дерева ((2,4)-дерева) поиска.
Ключ – целое число. Информация – строка произвольной длины.
Узел дерева содержит ключ, массив указателей на поддеревья и указатель на информационное поле. 
При необходимости в узел дерева может быть включен указатель на родительский узел.
...
возврат элемента, следующего за элементом с заданным значением ключа.
Примечания:
...
(*) Для целей отладки реализовать форматированный вывод таблицы в виде дерева.
